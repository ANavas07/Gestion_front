...
@Component({
  selector: 'app-modal-category-employee',
  templateUrl: './modal-category-employee.component.html',
  styleUrls: ['./modal-category-employee.component.css']
})
export class ModalCategoryEmployeeComponent {
  constructor(
  ...
  formAddCategory = new FormGroup({
    ...
    nameCat: new FormControl('', [
      Validators.required,
      Validators.maxLength(10),
      Validators.pattern(/^[a-zA-ZñÑ\s]*$/)
    ]),
    descriptionCat: new FormControl('', [
      Validators.required,
  ...
  addCategory(modalName: string) {
    ...
    this._categoriesService.addCategory(category).subscribe({
      next: (v) => {
        this.toastr.success(v.msg, 'Éxito!');
        setTimeout(() => {
